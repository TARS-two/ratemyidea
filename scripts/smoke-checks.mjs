...
assert(home.includes('inline-share-card-preview') && home.includes('Tu card está lista') && home.includes('Your card is ready'), 'Basic results should include a compact inline share-card preview with lighter copy.');
assert(home.includes('Share score card') && home.includes('Download score card image') && home.includes('Copy link and text'), 'Inline share-card actions should keep accessible full labels while showing compact button text.');
assert(home.includes('max-w-[20rem]') && home.includes('lg:grid-cols-[minmax(0,0.64fr)_minmax(0,1fr)]'), 'Inline share-card preview should stay visually compact, especially on mobile.');
