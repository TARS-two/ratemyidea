...
assert(marketStudyPreview.includes('h-[100dvh]') && marketStudyPreview.includes('overflow-y-auto') && marketStudyPreview.includes('blur-[3px]'), 'Market Study preview should be a full-height mobile/internal-scroll PDF-style preview with blurred locked sections.');
...
assert(marketStudyPreview.includes('AI Norte Research Brief'), 'Market Study preview should carry AI Norte branded report framing, not just app UI chrome.');
...
assert(!marketStudyPreview.includes('Qué cambia vs. el análisis básico') && !marketStudyPreview.includes('What changes vs. the basic analysis'), 'Market Study preview should remove the separate what-changes explainer card to reduce nested mobile boxes.');
assert(marketStudyPreview.includes('marketStudyBackButton') && marketStudyPreview.includes('Volver al análisis básico') && marketStudyPreview.includes('Back to basic analysis'), 'Market Study preview should include a clear always-visible in-page/back-to-basic button instead of relying on browser back.');
assert(marketStudyPreview.includes('h-[100dvh]') && marketStudyPreview.includes('pb-[calc(1rem+env(safe-area-inset-bottom))]'), 'Market Study preview should use dynamic viewport height and safe-area bottom padding so the sticky checkout CTA is not clipped on tall Android phones.');
...
assert(home.includes('proBenchmarkPreviewHeader') && home.includes('proBenchmarkPreviewHero') && home.includes('bg-gradient-to-b') && home.includes('Pro</span>') && !home.includes('Included with Pro') && !home.includes('Incluido con Pro') && !home.includes('border-t border-white/10 bg-[var(--surface)] p-6'), 'Locked Pro benchmark preview should put title/badges at the top and transition naturally from visual blur to text without a hard separator line.');
